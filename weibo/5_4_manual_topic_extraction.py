...
def load_stopwords():
    """加载停用词"""
    stopwords_file = "分词/baiduStopwords.txt"
    stopwords = set()
        ...
        with open(stopwords_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if word and not word.startswith('#'):
                    stopwords.add(word)
        print(f"成功加载停用词 {len(stopwords)} 个")
    except FileNotFoundError:
        print(f"警告: 停用词文件 {stopwords_file} 未找到，将使用默认停用词")
        # 添加一些常见的非实词
        default_stopwords = {
            '可能', '话筒', '第一', '晚安', '真的', '没有', '记者', '事儿', '回复',
            '一个', '一些', '一直', '许多', '有的', '这个', '那个', '什么', '怎么',
            '为什么', '因为', '所以', '但是', '然后', '现在', '今天', '明天', '昨天',
            '时候', '地方', '东西', '事情', '问题', '情况', '时间', '地方', '方面',
            '时候', '地方', '东西', '事情', '问题', '情况', '时间', '地方', '方面'
        }
...
def merge_similar_keywords(keywords, similarity_threshold=0.8):
    """合并相似的关键词（限制为3个字符以内）"""
    if not keywords:
        return keywords
    
    # 首先过滤掉超过3个字符的关键词
    filtered_keywords = [kw for kw in keywords if 1 <= len(kw) <= 3]
    ...
    for keyword in sorted_keywords:
        # 检查是否与已合并的关键词相似
        is_similar = False
        for i, merged in enumerate(merged_keywords):
            # 检查包含关系
            if keyword in merged or merged in keyword:
                # 如果较短的词在较长的词中，且相似度较高，则跳过
                if len(keyword) < len(merged) and len(keyword) / len(merged) > similarity_threshold:
                    is_similar = True
                    break
                # 如果较长的词包含较短的词，则替换
                elif len(keyword) > len(merged) and merged in keyword:
                    merged_keywords[i] = keyword
                    is_similar = True
                    break
                # 如果两个词完全相同，跳过
                elif keyword == merged:
                    is_similar = True
                    break
                # 特殊处理：如果一个是另一个的子串，保留较长的
                elif keyword in merged and len(keyword) < len(merged):
                    is_similar = True
                    break
                elif merged in keyword and len(merged) < len(keyword):
                    merged_keywords[i] = keyword
                    is_similar = True
                    break
...
def extract_topic_keywords(texts, topics, top_n=20):
    ...
    print(f"使用停用词过滤，共 {len(stopwords)} 个停用词")
    ...
    for topic_id, docs in topic_documents.items():
        ...
        try:
            ...
            # 获取top_n关键词
            top_indices = np.argsort(tfidf_scores)[::-1][:top_n*2]  # 获取更多候选词
            raw_keywords = [(feature_names[i], tfidf_scores[i]) for i in top_indices if tfidf_scores[i] > 0]
            
            # 过滤停用词和长词（限制为3个字符以内）
            filtered_keywords = []
            for word, score in raw_keywords:
                if word not in stopwords and 1 <= len(word) <= 3:  # 过滤停用词，限制为1-3个字符
                    filtered_keywords.append((word, score))
            
            # 合并相似关键词
            if filtered_keywords:
                keyword_words = [word for word, score in filtered_keywords]
                merged_words = merge_similar_keywords(keyword_words)
                
                # 重新构建关键词列表，保持分数
                final_keywords = []
                for word in merged_words:
                    # 找到对应的分数
                    for original_word, score in filtered_keywords:
            ...
            topic_keywords[topic_id] = final_keywords
            
            # 记录主题信息
            topic_info.append({
                'Topic': topic_id,
                'Count': len(docs),
                'Keywords': ', '.join([word for word, score in final_keywords[:10]]),
                'Top_Keywords': final_keywords[:10]
            })
            
            print(f"聚类{topic_id}: {len(docs)}个文档, 关键词: {', '.join([word for word, score in final_keywords[:5]])}")
...
def create_topic_word_barcharts(topic_info, viz_dir, top_n=10):
    ...
    for idx, info in enumerate(show_topics):
        ...
        ax.set_title(info['Top_Keywords'][0][0] if info['Top_Keywords'] else f"主题{info['Topic']}", fontsize=14, fontweight='bold')
...
def create_topic_barchart(topic_info, viz_dir):
    ...
    topic_labels = [info['Top_Keywords'][0][0] if info['Top_Keywords'] else f"主题{info['Topic']}" for info in topic_info]
...
def create_topic_visualization(topic_info, embeddings, topics, viz_dir):
    try:
        ...
        for info in topic_info:
            topic_id2label[info['Topic']] = info['Top_Keywords'][0][0] if info['Top_Keywords'] else f"主题{info['Topic']}"
        ...
        for i, topic_id in enumerate(unique_topics):
            ...
            if topic_id == -1:
                plt.scatter(reduced_embeddings[mask, 0], reduced_embeddings[mask, 1], c='gray', alpha=0.3, s=20, label='异常值')
            else:
                plt.scatter(reduced_embeddings[mask, 0], reduced_embeddings[mask, 1], c=[colors[i]], alpha=0.7, s=30, label=label)
...
def create_document_visualization(texts, embeddings, topics, viz_dir, topic_info):
    try:
        ...
        for i, topic_id in enumerate(unique_topics):
            ...
            label = topic_id2label.get(topic_id, f"主题{topic_id}")
            if topic_id == -1:
                plt.scatter(reduced_embeddings[mask, 0], reduced_embeddings[mask, 1], c='lightgray', alpha=0.4, s=15, label='异常值')
            else:
                plt.scatter(reduced_embeddings[mask, 0], reduced_embeddings[mask, 1], c=[colors[i]], alpha=0.6, s=20, label=label)
        plt.xlabel('UMAP Component 1', fontsize=14)
        plt.ylabel('UMAP Component 2', fontsize=14)
    ...
    try:
        ...
        for info in topic_info:
            ...
            for word, score in info['Top_Keywords']:
                words_data.append({'Topic': info['Top_Keywords'][0][0] if info['Top_Keywords'] else f"主题{topic_id}", 'Word': word, 'Score': score})
        if words_data:
...
def create_topic_probability_distribution(topic_info, viz_dir):
    try:
        ...
        wedges, texts, autotexts = plt.pie(probabilities, labels=topic_labels, autopct='%1.1f%%', colors=colors, startangle=90, pctdistance=0.85)
...
def create_topic_similarity_visualization(topic_info, embeddings, topics, viz_dir):
    try:
        ...
        if len(topic_centers) > 1:
            ...
            sns.heatmap(similarity_matrix, annot=True, cmap='Reds', center=0.5, xticklabels=topic_labels, yticklabels=topic_labels, fmt='.1f', annot_kws={"size":8})
