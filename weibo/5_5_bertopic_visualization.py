...
def main():
    ...
    topic_info['New_Topic'] = range(len(topic_info))
        ...
        if len(unique_topics) <= 4:
            ...
            if len(existing_topics) >= 4:
                ...
                if len(existing_topics) >= 20:
                    merge_config = [
                        [19, 2] if 19 in existing_topics and 2 in existing_topics else [existing_topics[0],
                                                                                        existing_topics[1]],
                        [9, 6, 1, 12, 20, 5, 15, 18] if all(
                            t in existing_topics for t in [9, 6, 1, 12, 20, 5, 15, 18]) else existing_topics[2:10],
                        [11, 16, 4, 10, 7] if all(
                            t in existing_topics for t in [11, 16, 4, 10, 7]) else existing_topics[10:15],
                        [17, 13, 0, 3, 8, 14] if all(
                            t in existing_topics for t in [17, 13, 0, 3, 8, 14]) else existing_topics[15:21]
                    ]
                else:
                    # 如果主题数量不够，分成4组
                ...
                try:
                    ...
                    fig = px.scatter(
                        ...
                        text=[merged_topic2label.get(info['Topic'], f"主题{info['Topic']}") for info in
                              merged_topic_info],
                        title='合并主题后的主题分布',
