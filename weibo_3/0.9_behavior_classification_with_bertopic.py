...
class BERTopicBehaviorClassifier:
    ...
    def _build_behavior_keywords(self):
        ...
        logger.info(
            f"行为关键词词典构建完成，包含 {sum(len(cat) for behavior in behavior_keywords.values() for cat in behavior.values())} 个关键词")
    ...
    def load_data(self, data_path, max_samples=1000):
        ...
        try:
            ...
            if len(df) > max_samples:
                logger.info(f"数据量过大({len(df)})，只取前{max_samples}条进行测试...")
    ...
    def train_bertopic_model(self, texts, embedding_model_name='paraphrase-multilingual-MiniLM-L12-v2',
        ...
        if len(texts) > sample_size:
            logger.info(f"数据量过大({len(texts)})，随机采样{sample_size}条进行训练...")
            import random
            random.seed(42)
        ...
        # 训练BERTopic模型（修复版本兼容性问题）
        try:
            self.topic_model = BERTopic(
                nr_topics="auto",
                min_topic_size=10,  # 减少最小主题大小
                verbose=True,
